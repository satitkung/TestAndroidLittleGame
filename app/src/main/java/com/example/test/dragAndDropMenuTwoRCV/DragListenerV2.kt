...
class DragListenerV2 : OnDragListener {
                ...
                when (viewId) {
                    flItem, rvTop, rvBottom -> {
                        ...
                        if (viewSource != null) {
                            ...
                            val adapterSource = source.adapter as ListAdapterV2
                            ...
                            //section target
                            val adapterTarget = target.adapter as ListAdapterV2
                            val targetId = target.id
                            val listTarget = adapterTarget.getList().toMutableList()
...
//                                adapterSource.updateList(listSource)
                                adapterSource.submitList(listSource.toList())
                                if (positionTarget >= 0) {
...
//                                adapterTarget.updateList(listTarget)
                                adapterTarget.submitList(listTarget.toList())
                            } else {
                                listSource.removeAt(positionSource)
//                                adapterSource.updateList(listSource)
                                adapterSource.submitList(listSource.toList())
